...
Avatar represents a person or entity with an image or a stable, name-derived fallback. When an image is missing or fails to load, Avatar displays the first meaningful character of the name on a deterministic gradient.
...
The gradient is derived from the normalized name rather than generated randomly at render time. The same identity therefore keeps the same treatment across sessions, themes and server rendering.
