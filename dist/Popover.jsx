...
export function PopoverTrigger({ PopoverComponent, jss, dialogJSStyle, grow, shrink, tag, children, }) {
    ...
    return (<BaseView grow={grow} shrink={shrink} tag={tag} relative={true} jss={[jss, { position: "relative", display: "inline-block" }]}>
      {children({ toggle })}
      <dialog ref={(ref) => {
            dialogRef.current = ref;
        }} className={toClassnames([jsss.popover, dialogJSStyle])} onClose={() => {
            setPopover(null);
        }} onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
