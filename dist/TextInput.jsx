...
export const TextInput = React.forwardRef(({ value, onValueChange, rootJSStyle, jss, addOn, gap = "small", addOnPosition = "start", padding = "medium", bare, ...inputProps }, ref) => {
    return (<Row gap={gap} jss={[
        ...
        {addOnPosition === "start" && addOn && (<Column justify="center" align="center" jss={[getPadding(padding), { paddingRight: 0 }]}>
            {addOn}
          </Column>)}
        <BaseInput {...inputProps} ref={ref} value={value} onChange={onValueChange ? (e) => onValueChange(e.target.value) : undefined} jss={[
            {
                minWidth: 0,
                flexGrow: 1,
                backgroundColor: "inherit",
        ...
        {addOnPosition === "end" && addOn && (<Column justify="center" align="center" jss={[getPadding(padding), { paddingLeft: 0 }]}>
            {addOn}
          </Column>)}
      </Row>);
});
