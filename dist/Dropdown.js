...
import { getBackground, createJSStyles, grow } from "./Styles";
...
export function Dropdown({ id = guid(), label, options, selection, setSelection, jsStyle, }) {
    ...
    return (React.createElement(BaseView, { ref: rootRef, jsStyle: [jsStyles.root, grow, jsStyle] },
        ...
        expanded && (React.createElement(Column, { gap: "medium", jsStyle: [jsStyles.dropdown, getBackground("secondary-background")], id: id, role: "listbox", "aria-label": label, tabIndex: -1, ref: dropdownRef, onBlur: (e) => {
            ...
            return (React.createElement(Button, { justify: selected ? "space-between" : "flex-start", align: "center", bare: true, iconPosition: "right", icon: selected ? "fa-check" : undefined, key: optionId, role: "option", id: optionId, "aria-selected": selected ? "true" : "false", label: option.toString(), color: "positive", onPress: () => {
                    setSelection(option);
                    setExpanded(false);
                } }));
        })))));
