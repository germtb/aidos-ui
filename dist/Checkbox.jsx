...
export function Checkbox({ checked, onClick, size, ...inputProps }) {
    return (<label className={toClassnames({
            position: "relative",
            display: "flex",
      ...
      <BaseInput {...inputProps} type="checkbox" role="checkbox" aria-checked={`${checked}`} tabIndex={0} checked={checked} onChange={onClick} jss={{
            position: "absolute",
            opacity: 0,
            cursor: "pointer",
            top: 0,
