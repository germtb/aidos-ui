...
export const BaseButton = React.forwardRef(({ componentName, onPress, children, jsStyle, color, bare = false, disabled, animateClick = true, padding, ...otherProps }, ref) => {
    return (<button {...otherProps} data-test-id={componentName ?? "BaseButton"} aria-disabled={disabled ? true : undefined} 
    ...
    ref={ref} onClick={(event) => {
            ...
            onPress(event);
        }} className={createClassNames(jsStyles.root, color === "positive" && jsStyles.positive, color === "secondary" && jsStyles.secondary, color === "negative" && jsStyles.negative, bare && jsStyles.bare, disabled && jsStyles.disabled, !bare && jsStyles.opacityHover, animateClick && !disabled && jsStyles.animateClick, jsStyle, getPadding(padding))}>
        {children}
      </button>);
});
