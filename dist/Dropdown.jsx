...
export function Dropdown({ id = guid(), label, options, selection, setSelection, jsStyle, }) {
    ...
    return (<BaseView ref={rootRef} jsStyle={[jsStyles.root, jsStyle]}>
      <Button aria-expanded={expanded ? "true" : "false"} aria-controls={id} aria-haspopup="true" aria-label={label} role="combobox" justify="space-between" tabIndex={0} jsStyle={jsStyles.button} color="positive" icon="fa-chevron-down" iconPosition="right" onClick={() => {
            setExpanded((x) => !x);
        }} ref={triggerRef}>
        {selection.toString()}
      </Button>
      {expanded && (<Column gap="medium" jsStyle={[jsStyles.dropdown, getBackground("secondary-background")]} id={id} role="listbox" aria-label={label} tabIndex={-1} ref={dropdownRef} onBlur={(e) => {
          ...
          {Array.from(options).map((option, index) => {
                ...
                return (<Button justify={selected ? "space-between" : "flex-start"} align="center" bare={true} iconPosition="right" icon={selected ? "fa-check" : undefined} key={optionId} role="option" id={optionId} aria-selected={selected ? "true" : "false"} color="positive" onClick={() => {
                        setSelection(option);
                        setExpanded(false);
                    }}>
                {option.toString()}
